...
| Embeddings | **Live** — a self-hosted `embedder` sidecar (HF text-embeddings-inference serving `multilingual-e5-small`, 384-dim, OpenAI-compatible) runs at €0. Semantic search works in the UI (**Search** page) and via `GET /api/v1/captures/search`; the reset cycle re-embeds the seeded fixtures each round so they're findable. Where no embedder is configured the endpoint returns 503 ProblemDetails (transparent). |
