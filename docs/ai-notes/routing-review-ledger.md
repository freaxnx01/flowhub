...
- **status:** fixed — `RepoResolver` now forces `ideas-lab` for every `BridgeAction.Idea` (test `ResolveAsync_ModelPicksARepoForAnIdea_TargetsIdeasLabAnyway`). Repo-side only; CT 136 keeps the old behaviour until redeployed.
