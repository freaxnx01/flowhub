...
const DEMO_SCHEMA = {
  ...
  properties: {
    ...
    embeddingsLive: { type: 'boolean', description: 'did /api/v1/captures/search return 200 with semantically-ranked results (self-hosted embedder live in the demo)' },
...
const demoPromise = agent(
  [
    ...
    '4. Embeddings posture: GET ' + demoUrl + '/api/v1/captures/search?q=paper → expect HTTP 200 with a JSON array of semantically-ranked captures (the demo self-hosts a multilingual embedder; if it is still warming up just after a deploy the endpoint may briefly 503, then recovers).',
