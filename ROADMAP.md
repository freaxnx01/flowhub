...
1. **New driven adapter** `GitHubIssueSkillIntegration : ISkillIntegration` — resolves the target repo from the capture prefix (`flowhub:` → `freaxnx01/flowhub`) and POSTs to the GitHub Issues API; `ExternalRef` = issue number/URL.
...
|---|---|---|---|---|
| **G1** | LLM02 — Sensitive Information Disclosure | [#159](https://github.com/freaxnx01/flowhub/issues/159) | `AiClassifier` sends Capture content **verbatim** to the provider (OpenRouter). No outbound redaction/PII guardrail before content leaves the process. | Any real-data (non-anonymous) deployment |
| **G2** | LLM08 — Vector & Embedding Weaknesses | [#160](https://github.com/freaxnx01/flowhub/issues/160) | Capture reads + `SearchByEmbeddingAsync` are **not tenant-scoped** — any authenticated caller can read/search all captures. *Highest-priority residual gap.* | Multi-user (OIDC, Block 5) |
| **G3** | LLM10 — Unbounded Consumption | [#161](https://github.com/freaxnx01/flowhub/issues/161) | The OpenRouter **$1/mo cap is account-side, not code-enforced**; the only in-repo control is the Traefik rate-limit. Needs an operator runbook making the split explicit. | Now (doc-only) |
| **G4** | ASI03 — Identity & Privilege Abuse | [#162](https://github.com/freaxnx01/flowhub/issues/162) | Each `ISkillIntegration` acts with a static, system-scoped token; no end-user identity is delegated. Confused-deputy surface needs review once real auth exists. | Block 5 OIDC |
